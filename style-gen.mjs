...
${style_table.map(([typeName, fieldName]) => `\t${typeName} ${fieldName};`).join("\n")}
} box_style_t;
`

style_header += `
typedef uint64_t box_style_flag_t;
${style_table.map(([, fieldName], index) => `#define BOX_STYLE_${fieldName} (UINT64_C(1) << ${index})`).join("\n")}
...
let count = 0
style_table.forEach(([typeName, fieldName, supportAuto, supportContent], index) => {
    if (["FlexWrapMode", "FlexDirection", "FlexAlign", "float", "plutovg_color_t", "TEXT_ALIGN", "double", "CSS_OVERFLOW", "int"]
        .includes(typeName)) {
        count = count + 1
        style_header += GenBoxStyleDeclaration(fieldName, `${typeName} ${fieldName}`)
    }
    else if (typeName === "char *") {
        count = count + 1
        style_header += GenBoxStyleDeclaration(fieldName, `const char *${fieldName}`)
    }
    else if (typeName === "plutovg_matrix_t") {
        count = count + 1
        style_header += GenBoxStyleDeclaration(fieldName, "double m00", "double m10", "double m01", "double m11", "double m02", "double m12")
    }
    else if (typeName === "struct transform_origin_t") {
        count = count + 1
        style_header += GenBoxStyleDeclaration(fieldName + "Keyword", "TRANSFORM_ORIGIN x", "TRANSFORM_ORIGIN y")
        style_header += GenBoxStyleDeclaration(fieldName + "Offset", "double x", "double y")
    }
    else if (typeName === "FlexLength") {
        count = count + 1
        style_header += GenBoxStyleDeclaration(fieldName, `float ${fieldName}`)
        style_header += GenBoxStyleDeclaration(fieldName + "Percent", `float ${fieldName}`)
        if (supportAuto) {
...
style_table.forEach(([typeName, fieldName, supportAuto, supportContent], index) => {
    if (["FlexWrapMode", "FlexDirection", "FlexAlign", "float", "plutovg_color_t", "TEXT_ALIGN", "double", "CSS_OVERFLOW", "int"]
        .includes(typeName)) {
        count = count + 1
        style_source += GenBoxStyleDefine2(fieldName, index, `${typeName} ${fieldName}`, `style->${fieldName} = ${fieldName};`)
    }
    else if (typeName === "char *") {
        count = count + 1
        style_source += GenBoxStyleDefine2(fieldName, index, `const char *${fieldName}`, `assert(${fieldName});
    if (style->${fieldName}) free(style->${fieldName});
    style->${fieldName} = strdup(${fieldName});`)
    }
    else if (typeName === "plutovg_matrix_t") {
        count = count + 1
        style_source += GenBoxStyleDefine2(fieldName, index, "double m00", "double m10", "double m01", "double m11", "double m02", "double m12",
            `plutovg_matrix_init(&style->${fieldName}, m00, m10, m01, m11, m02, m12);`)
    }
    else if (typeName === "struct transform_origin_t") {
        count = count + 1
        style_source += GenBoxStyleDefine2(fieldName + "Keyword", index, "TRANSFORM_ORIGIN x", "TRANSFORM_ORIGIN y", `style->${fieldName}.type = TRANSFORM_ORIGIN_TYPE_KEYWORD;
    style->${fieldName}.x.keyword = x;
    style->${fieldName}.y.keyword = y;`)
        style_source += GenBoxStyleDefine2(fieldName + "Offset", index, "double x", "double y", `style->${fieldName}.type = TRANSFORM_ORIGIN_TYPE_OFFSET;
    style->${fieldName}.x.offset = x;
    style->${fieldName}.y.offset = y;`)
    }
    else if (typeName === "FlexLength") {
        count = count + 1
        style_source += GenBoxStyleDefine2(fieldName, index, `float ${fieldName}`, `style->${fieldName}.value = ${fieldName};
    style->${fieldName}.type = FlexLengthTypePoint;`)
        style_source += GenBoxStyleDefine2(fieldName + "Percent", index, `float ${fieldName}`, `style->${fieldName}.value = ${fieldName};
    style->${fieldName}.type = FlexLengthTypePercent;`)
        if (supportAuto) {
            style_source += GenBoxStyleDefine2(fieldName + "Auto", index, `style->${fieldName} = FlexLengthAuto;`)
        }

        if (supportContent) {
            style_source += GenBoxStyleDefine2(fieldName + "Content", index, `style->${fieldName} = FlexLengthContent;`)
        }
    }
})
...
void box_style_merge(box_style_t *dst, const box_style_t *src)
{
    ...
    for(int i = 0; i < ${style_table.length}; i++)
    {
        ...
        switch(i)
        {
${style_table.map(([typeName, fieldName], index) => {
...
`
    ...
    else if (["FlexLength"].includes(typeName) && !["left", "right", "top", "bottom"].includes(fieldName)) {
        return `
            case ${index}:
