...
static text_lru_key_t *key_alloc(const char *font_family, size_t path_len, int ch, double font_size)
{
...
static int lru_get_by_text_key(struct lru_t *lru, text_cache_t *cache, const char *font_family, int ch, double font_size)
{
    size_t len = strlen(font_family);
    text_lru_key_t *key = key_alloc(font_family, len, ch, font_size);
    int ret = lru_get(lru, (const char *)key, sizeof(text_lru_key_t) + len, cache, sizeof(text_cache_t));
    free(key);
    return ret;
}

static int lru_set_by_text_key(struct lru_t *lru, const char *font_family, int ch, double font_size, text_cache_t *cache)
{
    size_t len = strlen(font_family);
    text_lru_key_t *key = key_alloc(font_family, len, ch, font_size);
...
{
    text_cache_t cache;
    int ret = lru_get_by_text_key(lru, &cache, font_family, ch, font_size);
    if (ret)
    {
...
static plutovg_surface_t *__text_lru_set(struct lru_t *lru, const char *font_family, int ch, double font_size)
{
    ...
    text_cache_t cache;
    cache.text = text;
    lru_set_by_text_key(lru, font_family, ch, font_size, &cache);
    return text;
}

int text_lru_set(struct lru_t *lru, const char *font_family, int ch, double font_size)
{
    return __text_lru_set(lru, font_family, ch, font_size) != NULL;
}

plutovg_surface_t *text_lru_load(struct lru_t *lru, const char *font_family, int ch, double font_size)
{
    plutovg_surface_t *text = text_lru_get(lru, font_family, ch, font_size);

    if (text == NULL)
    {
        text = __text_lru_set(lru, font_family, ch, font_size);
